...
contract AcyV1Router02 is IAcyV1Router02 {
    ...
    mapping(address => address) priceMap;
    
    constructor() public {
        factory = 0x6A7e3487A5ed6f437984683f33A71a05ad1D9D1C;
        WETH = 0x6A7e3487A5ed6f437984683f33A71a05ad1D9D1C;
        
        priceMap[0x038621452E421E469CfEE7eaD83f92482bCF21DD] = 0x007A22900a3B98143368Bd5906f8E17e9867581b;
        priceMap[0x8acC0655ca71f100d362b667F89Acc880ca3EED8] = 0x0715A7794a1dc8e42615F059dD6e406A6594651A;
    ...
    address[3] public middleTokenList = [
        0x038621452E421E469CfEE7eaD83f92482bCF21DD, // Token WBTC
        0x8acC0655ca71f100d362b667F89Acc880ca3EED8, // Token WETH
        0x5a06c836FD008Ea0021FC4398477DEa70c7a6A58  // Token WMATIC
    ];
    
    AggregatorV3Interface priceFeedIn;
    AggregatorV3Interface priceFeedOut;
    ...
    function getThePrice(address token) public view returns (int) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(priceMap[token]);
        (
            uint80 roundID, 
            int price,
            uint startedAt,
    ...
    function swapExactTokensForTokens(
        ...
        for (uint i = 0; i < 3; i++) {
            if (middleTokenList[i] != path[0] && middleTokenList[i] != path[1]) {
                ...
                uint[] memory amountsDir = this.swapExactTokensForTokensPath(
                            dirAmount,
                            0,
                            path,
                            to,
                ...
                uint[] memory amountsArb = this.swapExactTokensForTokensPath(
                            amountIn - dirAmount,
                            0,
                            arbPath,
                            to,
                            now.add(1800),
                            msg.sender);
                uint amountIn = amountsDir[0] + amountsArb[0];
                uint amountOut = amountsDir[amountsDir.length - 1] + amountsArb[amountsArb.length - 1];
                
                uint priceIn = uint(getThePrice(path[0]));
                
                uint priceOut = uint(getThePrice(path[1]));
                
                require (amountIn * priceIn / amountOut / priceOut < 2, "chainlink price protection");
            }
        }
